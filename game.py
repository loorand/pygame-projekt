...
def scores():
    ...
    if player_rect.colliderect(energy_rect):
        ...
        energy_rect.left = random.randint(0, random.randint(0, screen_x - energy_x))
        energy_collect += 1
        score += 100
    score -= 1
    y = 0 # y-koordinaadi muutuja skoorile
    if player_rect.top < 100:
        y = player_rect.top - 100 - (80 - player_rect.top) * .5

    score_surface = font.render(f"Energy: {(score // 10)}", True, "#bbbbbb").convert_alpha()
    score_rect = score_surface.get_rect(center = (216, y + 30))
    screen.blit(score_surface, score_rect)
    energy_col_surface = font.render(f"Score: {energy_collect}", True, "#bbbbbb").convert_alpha()
    energy_col_rect = energy_col_surface.get_rect(center = (216, y + 70))
    screen.blit(energy_col_surface, energy_col_rect)

    score2_surface = font.render(f"Energy: {(score // 10)}", True, "#bbbbbb").convert_alpha()
    score2_rect = score2_surface.get_rect(center = (216, y + 30 + screen.get_height()))
    screen.blit(score2_surface, score2_rect)
    energy2_col_surface = font.render(f"Score: {energy_collect}", True, "#bbbbbb").convert_alpha()
...
pygame.display.set_caption("Moving Into Space")
...
barrel_x = barrel_surface.get_width()
barrel_rect = barrel_surface.get_rect(bottomleft = (random.randint(0, screen_x - barrel_x), random.randint(-20,-10) * 10))
# duck surface / spawn location
duck_surface = pygame.image.load("graphics/duck.png").convert_alpha()
duck_surface.set_alpha(200)
duck_x = duck_surface.get_width()
duck_rect = duck_surface.get_rect(bottomleft = (random.randint(0, screen_x - duck_x), random.randint(-40,-20) * 10))
...
energy_x = energy_surface.get_width()
energy_rect = energy_surface.get_rect(bottomleft = (random.randint(0, screen_x - energy_x), random.randint(-30,-15) * 10))
...
game = False
sound = "MUTE" # sound on by default, toggle to MUTE/UNMUTE
menu_music()
# main loop
while True:
    ...
    if game:
        ...
        if barrel_rect.top >= screen.get_height():
            ...
            barrel_rect.left = random.randint(0, random.randint(0, screen_x - barrel_x))
        ...
        if duck_rect.top >= screen.get_height():
            ...
            duck_rect.left = random.randint(0, random.randint(0, screen_x - duck_x))
        ...
        screen.blit(bg_surface, bg_rect)
        screen.blit(bg1_surface, bg1_rect)
        draw_text("Moving Into Space", font, "#bbbbbb", screen, 40)
